...
let
    ...
    plot!(-1:0.09:1, x->imag(k3b((1e3*ΓDˣ⁰)*x)/real(k3b(0.001))))
    plot!(-1:0.09:1, x->imag(kNR((1e3*ΓDˣ⁰)*x)/real(kNR(0.001))))
    plot!(-1:0.09:1, x->imag(ρ₀( (1e3*ΓDˣ⁰)*x)/real(ρ₀( 0.001))))
end
...
let
    plot(size=(1000,300), layout=grid(1,3),
        ...
        colorbar=false)
    heatmap!(sp=1, -1:0.1:1, -1:0.1:1,   (x,y)->real(k3b(Eᵦ+(1e3*ΓDˣ⁰)*(x+1im*y))))
    heatmap!(sp=2, -1:0.1:1, -1:0.1:1,   (x,y)->real(kNR(Eᵦ+(1e3*ΓDˣ⁰)*(x+1im*y))))
    heatmap!(sp=3, -1:0.09:1, -1:0.09:1, (x,y)->real(ρ₀( Eᵦ+(1e3*ΓDˣ⁰)*(x+1im*y))))
end
